	...
	for fn in sys.argv[2:]:
		...
		for x in range(5):
			...
			for i in range(n):
				#…wordfreqs--it is going to print n words from wordfreqs, which will be all
				#the words that were in the tweets after cleaning and after popping the stop words.
				print rand(wordfreqs),' ',rand(stopwords),' '
			print
...
#command for all files: python twitterFrequency.py tweets_corpus_all.txt tweets_corpus1.txt tweets_corpus2.txt tweets_corpus3.txt tweets_corpus4.txt tweets_corpus5.txt tweets_corpus_classchoice.txt tweets_corpus_mystery.txt 
